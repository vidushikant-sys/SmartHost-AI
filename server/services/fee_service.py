"""
Fee Management Service - Production Ready
Handles all fee and payment operations for the hostel management system.
"""

from datetime import date, datetime
from sqlalchemy import func

from config.extensions import db
from models.fee import Fee
from models.fee_payment import FeePayment
from models.student import Student
from models.room import Room
from models.property import Property
from validators.fee_validator import FeeValidator


# ============================================================
# Constants
# ============================================================

VALID_PAYMENT_METHODS = ["Cash", "UPI", "Card", "Bank Transfer"]


# ============================================================
# Fee Management
# ============================================================

def create_fee(data):
    """
    Create a new monthly fee for a student.
    
    Args:
        data: Dictionary containing fee details
        
    Returns:
        Fee object as dictionary
        
    Raises:
        ValueError: If validation fails or required entities not found
    """
    is_valid, errors = FeeValidator.validate(data)
    if not is_valid:
        raise ValueError(errors)

    student_id = data.get("student_id")
    hostel_id = data.get("hostel_id")
    room_id = data.get("room_id")
    month = data.get("month")
    year = data.get("year")
    due_date = data.get("due_date")
    monthly_rent = float(data.get("monthly_rent", 0))
    electricity_charge = float(data.get("electricity_charge", 0))
    water_charge = float(data.get("water_charge", 0))
    maintenance_charge = float(data.get("maintenance_charge", 0))
    other_charge = float(data.get("other_charge", 0))
    discount = float(data.get("discount", 0))
    fine = float(data.get("fine", 0))
    remarks = data.get("remarks")

    # Validate required entities
    if not Student.query.get(student_id):
        raise ValueError("Student not found.")
    
    if not Property.query.get(hostel_id):
        raise ValueError("Hostel not found.")
    
    if not Room.query.get(room_id):
        raise ValueError("Room not found.")

    # Check for duplicate fee
    existing_fee = Fee.query.filter_by(
        student_id=student_id,
        month=month,
        year=year
    ).first()
    if existing_fee:
        raise ValueError("Fee already generated for this month.")

    # Calculate total amount
    total_amount = (
        monthly_rent + electricity_charge + water_charge +
        maintenance_charge + other_charge + fine - discount
    )

    fee = Fee(
        student_id=student_id,
        hostel_id=hostel_id,
        room_id=room_id,
        month=month,
        year=year,
        due_date=due_date,
        monthly_rent=monthly_rent,
        electricity_charge=electricity_charge,
        water_charge=water_charge,
        maintenance_charge=maintenance_charge,
        other_charge=other_charge,
        discount=discount,
        fine=fine,
        total_amount=total_amount,
        paid_amount=0,
        remaining_amount=total_amount,
        payment_status="Pending",
        remarks=remarks
    )

    try:
        db.session.add(fee)
        db.session.commit()
    except Exception as e:
        db.session.rollback()
        raise ValueError(f"Failed to create fee: {str(e)}")

    return fee.to_dict()


def get_all_fees():
    """Get all fees ordered by year, month, and ID."""
    fees = Fee.query.order_by(
        Fee.year.desc(),
        Fee.month.desc(),
        Fee.id.desc()
    ).all()
    return [fee.to_dict() for fee in fees]


def get_fee_by_id(fee_id):
    """Get a specific fee by ID."""
    fee = Fee.query.get(fee_id)
    if not fee:
        raise ValueError("Fee not found.")
    return fee.to_dict()


def get_student_fee_history(student_id):
    """Get all fees for a specific student."""
    if not Student.query.get(student_id):
        raise ValueError("Student not found.")
    
    fees = Fee.query.filter_by(student_id=student_id).order_by(
        Fee.year.desc(),
        Fee.month.desc(),
        Fee.id.desc()
    ).all()
    return [fee.to_dict() for fee in fees]


def get_student_month_fee(student_id, month, year):
    """Get a student's fee for a specific month and year."""
    fee = Fee.query.filter_by(
        student_id=student_id,
        month=month,
        year=year
    ).first()
    if not fee:
        raise ValueError("Fee not found.")
    return fee.to_dict()


def get_pending_fees():
    """Get all pending fees (not fully paid)."""
    fees = Fee.query.filter(
        Fee.payment_status != "Paid"
    ).order_by(Fee.due_date.asc()).all()
    return [fee.to_dict() for fee in fees]


def get_paid_fees():
    """Get all fully paid fees."""
    fees = Fee.query.filter_by(payment_status="Paid").order_by(
        Fee.year.desc(),
        Fee.month.desc(),
        Fee.id.desc()
    ).all()
    return [fee.to_dict() for fee in fees]


def get_monthly_fees(month, year):
    """Get all fees for a specific month and year."""
    fees = Fee.query.filter_by(month=month, year=year).order_by(
        Fee.id.desc()
    ).all()
    return [fee.to_dict() for fee in fees]


def get_hostel_fees(hostel_id):
    """Get all fees for a specific hostel."""
    if not Property.query.get(hostel_id):
        raise ValueError("Hostel not found.")
    
    fees = Fee.query.filter_by(hostel_id=hostel_id).order_by(
        Fee.year.desc(),
        Fee.month.desc(),
        Fee.id.desc()
    ).all()
    return [fee.to_dict() for fee in fees]


def get_room_fees(room_id):
    """Get all fees for a specific room."""
    if not Room.query.get(room_id):
        raise ValueError("Room not found.")
    
    fees = Fee.query.filter_by(room_id=room_id).order_by(
        Fee.year.desc(),
        Fee.month.desc(),
        Fee.id.desc()
    ).all()
    return [fee.to_dict() for fee in fees]


def get_overdue_fees():
    """Get all overdue fees (due date passed and not fully paid)."""
    today = date.today()
    fees = Fee.query.filter(
        Fee.due_date < today,
        Fee.payment_status != "Paid"
    ).order_by(Fee.due_date.asc()).all()
    return [fee.to_dict() for fee in fees]


def update_fee(fee_id, data):
    """
    Update an existing fee.
    
    Raises:
        ValueError: If fee not found or validation fails
    """
    is_valid, errors = FeeValidator.validate_update(data)
    if not is_valid:
        raise ValueError(errors)

    fee = Fee.query.get(fee_id)
    if not fee:
        raise ValueError("Fee not found.")

    fee.monthly_rent = float(data.get("monthly_rent", fee.monthly_rent))
    fee.electricity_charge = float(data.get("electricity_charge", fee.electricity_charge))
    fee.water_charge = float(data.get("water_charge", fee.water_charge))
    fee.maintenance_charge = float(data.get("maintenance_charge", fee.maintenance_charge))
    fee.other_charge = float(data.get("other_charge", fee.other_charge))
    fee.discount = float(data.get("discount", fee.discount))
    fee.fine = float(data.get("fine", fee.fine))
    fee.due_date = data.get("due_date", fee.due_date)
    fee.remarks = data.get("remarks", fee.remarks)

    # Recalculate total amount
    fee.total_amount = (
        fee.monthly_rent + fee.electricity_charge + fee.water_charge +
        fee.maintenance_charge + fee.other_charge + fee.fine - fee.discount
    )

    fee.remaining_amount = fee.total_amount - fee.paid_amount

    # Update payment status
    if fee.remaining_amount <= 0:
        fee.remaining_amount = 0
        fee.payment_status = "Paid"
    elif fee.paid_amount == 0:
        fee.payment_status = "Pending"
    else:
        fee.payment_status = "Partial"

    try:
        db.session.commit()
    except Exception as e:
        db.session.rollback()
        raise ValueError(f"Failed to update fee: {str(e)}")

    return fee.to_dict()


def delete_fee(fee_id):
    """
    Delete a fee.
    
    Raises:
        ValueError: If fee not found
    """
    fee = Fee.query.get(fee_id)
    if not fee:
        raise ValueError("Fee not found.")

    try:
        db.session.delete(fee)
        db.session.commit()
    except Exception as e:
        db.session.rollback()
        raise ValueError(f"Failed to delete fee: {str(e)}")

    return {"message": "Fee deleted successfully."}


# ============================================================
# Payment Management
# ============================================================

def pay_fee(fee_id, data):
    """
    Process a fee payment (full or partial).
    
    Raises:
        ValueError: If validation fails or payment cannot be processed
    """
    is_valid, errors = FeeValidator.validate_payment(data)
    if not is_valid:
        raise ValueError(errors)

    fee = Fee.query.get(fee_id)
    if not fee:
        raise ValueError("Fee not found.")

    payment_amount = float(data.get("payment_amount", 0))
    if payment_amount <= 0:
        raise ValueError("Payment amount must be greater than zero.")
    if payment_amount > fee.remaining_amount:
        raise ValueError("Payment amount cannot exceed remaining amount.")

    payment_method = data.get("payment_method")
    if not payment_method:
        raise ValueError("Payment method is required.")

    payment_date = data.get("payment_date")
    if payment_date:
        try:
            payment_date = datetime.strptime(payment_date, "%Y-%m-%d").date()
        except ValueError:
            raise ValueError("Invalid payment date format. Use YYYY-MM-DD.")
    else:
        payment_date = date.today()

    transaction_id = data.get("transaction_id")
    received_by = data.get("received_by")
    remarks = data.get("remarks")

    payment = FeePayment(
        fee_id=fee.id,
        payment_amount=payment_amount,
        payment_method=payment_method,
        payment_date=payment_date,
        transaction_id=transaction_id,
        received_by=received_by,
        remarks=remarks
    )

    try:
        db.session.add(payment)
        fee.paid_amount += payment_amount
        fee.remaining_amount = fee.total_amount - fee.paid_amount

        # Update payment status
        if fee.remaining_amount <= 0:
            fee.remaining_amount = 0
            fee.payment_status = "Paid"
        elif fee.paid_amount == 0:
            fee.payment_status = "Pending"
        else:
            fee.payment_status = "Partial"

        db.session.commit()
    except Exception as e:
        db.session.rollback()
        raise ValueError(f"Failed to process payment: {str(e)}")

    return {
        "fee": fee.to_dict(),
        "payment": payment.to_dict()
    }


def add_payment(data):
    """
    Add a payment to a fee.
    
    Raises:
        ValueError: If validation fails or payment cannot be added
    """
    if data is None:
        raise ValueError("Request body is required.")

    is_valid, errors = FeeValidator.validate_payment(data)
    if not is_valid:
        raise ValueError(errors)

    fee_id = data.get("fee_id")
    fee = Fee.query.get(fee_id)
    if not fee:
        raise ValueError("Fee not found.")

    if fee.payment_status == "Paid":
        raise ValueError("This fee has already been paid.")

    payment_amount = float(data.get("payment_amount", 0))
    if payment_amount <= 0:
        raise ValueError("Payment amount must be greater than zero.")
    if payment_amount > fee.remaining_amount:
        raise ValueError("Payment amount cannot exceed remaining amount.")

    payment_method = data.get("payment_method")
    if payment_method not in VALID_PAYMENT_METHODS:
        raise ValueError("Invalid payment method.")

    payment_date = data.get("payment_date")
    if payment_date:
        try:
            payment_date = datetime.strptime(payment_date, "%Y-%m-%d").date()
        except ValueError:
            raise ValueError("Invalid payment date format. Use YYYY-MM-DD.")
    else:
        payment_date = date.today()

    payment = FeePayment(
        fee_id=fee.id,
        payment_amount=payment_amount,
        payment_date=payment_date,
        payment_method=payment_method,
        transaction_id=data.get("transaction_id"),
        received_by=data.get("received_by"),
        remarks=data.get("remarks")
    )

    try:
        db.session.add(payment)
        fee.paid_amount += payment_amount
        fee.remaining_amount = fee.total_amount - fee.paid_amount

        # Update payment status
        if fee.remaining_amount <= 0:
            fee.remaining_amount = 0
            fee.payment_status = "Paid"
        elif fee.paid_amount > 0:
            fee.payment_status = "Partial"
        else:
            fee.payment_status = "Pending"

        db.session.commit()
    except Exception as e:
        db.session.rollback()
        raise ValueError(f"Failed to add payment: {str(e)}")

    return payment.to_dict()


def get_payment(payment_id):
    """Get payment details with associated fee and student information."""
    payment = FeePayment.query.get(payment_id)
    if not payment:
        raise ValueError("Payment not found.")

    fee = Fee.query.get(payment.fee_id)
    if not fee:
        raise ValueError("Fee not found.")

    student = Student.query.get(fee.student_id)
    if not student:
        raise ValueError("Student not found.")

    return {
        "payment": payment.to_dict(),
        "fee": {
            "id": fee.id,
            "student_id": fee.student_id,
            "month": fee.month,
            "year": fee.year,
            "total_amount": fee.total_amount,
            "paid_amount": fee.paid_amount,
            "remaining_amount": fee.remaining_amount,
            "payment_status": fee.payment_status
        },
        "student": {
            "id": student.id,
            "name": student.full_name
        }
    }


def get_all_payments():
    """Get all payments with associated fee and student information."""
    payments = FeePayment.query.order_by(
        FeePayment.payment_date.desc(),
        FeePayment.id.desc()
    ).all()

    payment_list = []
    for payment in payments:
        fee = Fee.query.get(payment.fee_id)
        if not fee:
            continue

        student = Student.query.get(fee.student_id)
        if not student:
            continue

        payment_list.append({
            "payment": payment.to_dict(),
            "student": {
                "id": student.id,
                "name": student.full_name
            },
            "fee": {
                "id": fee.id,
                "month": fee.month,
                "year": fee.year,
                "total_amount": fee.total_amount,
                "paid_amount": fee.paid_amount,
                "remaining_amount": fee.remaining_amount,
                "payment_status": fee.payment_status
            }
        })

    return {
        "total_payments": len(payment_list),
        "payments": payment_list
    }


def delete_payment(payment_id):
    """
    Delete a payment and recalculate fee status.
    
    Raises:
        ValueError: If payment or fee not found
    """
    payment = FeePayment.query.get(payment_id)
    if not payment:
        raise ValueError("Payment not found.")

    fee = Fee.query.get(payment.fee_id)
    if not fee:
        raise ValueError("Fee not found.")

    try:
        db.session.delete(payment)
        db.session.flush()

        # Recalculate total paid amount
        total_paid = db.session.query(
            func.coalesce(func.sum(FeePayment.payment_amount), 0)
        ).filter(FeePayment.fee_id == fee.id).scalar()

        fee.paid_amount = float(total_paid)
        fee.remaining_amount = fee.total_amount - fee.paid_amount

        # Update payment status
        if fee.paid_amount <= 0:
            fee.paid_amount = 0
            fee.remaining_amount = fee.total_amount
            fee.payment_status = "Pending"
        elif fee.remaining_amount <= 0:
            fee.remaining_amount = 0
            fee.payment_status = "Paid"
        else:
            fee.payment_status = "Partial"

        db.session.commit()
    except Exception as e:
        db.session.rollback()
        raise ValueError(f"Failed to delete payment: {str(e)}")

    return {"message": "Payment deleted successfully."}


def get_payment_history(fee_id):
    """Get all payments for a specific fee."""
    fee = Fee.query.get(fee_id)
    if not fee:
        raise ValueError("Fee not found.")

    payments = FeePayment.query.filter_by(fee_id=fee_id).order_by(
        FeePayment.payment_date.desc(),
        FeePayment.id.desc()
    ).all()

    return {
        "fee": fee.to_dict(),
        "payments": [payment.to_dict() for payment in payments],
        "total_payments": len(payments)
    }


def get_student_payment_history(student_id):
    """Get payment history for all fees of a student."""
    student = Student.query.get(student_id)
    if not student:
        raise ValueError("Student not found.")

    fees = Fee.query.filter_by(student_id=student_id).all()
    payment_history = []

    for fee in fees:
        payments = FeePayment.query.filter_by(fee_id=fee.id).order_by(
            FeePayment.payment_date.desc(),
            FeePayment.id.desc()
        ).all()

        for payment in payments:
            payment_data = payment.to_dict()
            payment_data.update({
                "month": fee.month,
                "year": fee.year,
                "total_amount": fee.total_amount,
                "payment_status": fee.payment_status
            })
            payment_history.append(payment_data)

    # Sort by payment date descending, then by ID
    payment_history.sort(
        key=lambda x: (x["payment_date"], x["id"]),
        reverse=True
    )

    return payment_history


# ============================================================
# Reports
# ============================================================

def get_monthly_collection(month, year):
    """Get monthly collection report for a specific month and year."""
    total_collection = db.session.query(
        func.coalesce(func.sum(FeePayment.payment_amount), 0)
    ).join(Fee).filter(
        Fee.month == month,
        Fee.year == year
    ).scalar()

    total_fees = Fee.query.filter_by(month=month, year=year).count()

    return {
        "month": month,
        "year": year,
        "total_collection": float(total_collection),
        "total_fees": total_fees
    }


def get_fee_dashboard():
    """Get fee management dashboard statistics."""
    return {
        "total_fees": Fee.query.count(),
        "paid_fees": Fee.query.filter_by(payment_status="Paid").count(),
        "pending_fees": Fee.query.filter_by(payment_status="Pending").count(),
        "partial_fees": Fee.query.filter_by(payment_status="Partial").count(),
        "total_collection": float(
            db.session.query(
                func.coalesce(func.sum(FeePayment.payment_amount), 0)
            ).scalar()
        ),
        "total_due": float(
            db.session.query(
                func.coalesce(func.sum(Fee.remaining_amount), 0)
            ).scalar()
        )
    }
